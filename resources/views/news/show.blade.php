...
<body x-data="{
    ...
    <section class="relative min-h-[35vh] md:min-h-[45vh] bg-[#1a1a1a] flex items-end pt-20">
        <div class="absolute inset-0">
            @if($news->image)
            <img src="{{ asset('storage/' . $news->image) }}" alt="{{ $news->title }}" class="w-full h-full object-cover opacity-40">
            @else
                        ...
                        <a href="{{ route('news.show', $related->slug) }}" class="block group">
                            <div class="flex gap-4">
                                <div class="w-20 h-20 flex-shrink-0 overflow-hidden rounded-lg bg-gray-100">
                                    @if($related->image)
                                    <img src="{{ asset('storage/' . $related->image) }}" alt="{{ $related->title }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300">
                                    @else
