        ...
        <div class="relative z-10 h-full max-w-7xl mx-auto px-4 md:px-6 py-16 md:py-24 flex items-center justify-center text-center">
            <div>
                ...
                <div class="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-white text-sm font-medium mb-8 shadow-xl">
                    ...
                    {{ $programs->count() }} Program Keahlian Unggulan
                </div>
                ...
                <div class="flex flex-wrap justify-center gap-6 md:gap-12">
                    <div class="bg-white/10 backdrop-blur-md rounded-2xl px-6 py-4 border border-white/20">
                        <div class="text-3xl md:text-4xl font-bold text-white">{{ $programs->count() }}</div>
    ...
    <section class="py-16 md:py-24 bg-gradient-to-b from-gray-50 to-white">
        <div class="max-w-7xl mx-auto px-4 md:px-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-10">
                ...
                @php
                    $colorConfig = [
                        'indigo' => ['gradient' => 'from-indigo-500 to-blue-600', 'bg' => 'bg-indigo-100/50', 'text' => 'text-indigo-600', 'shadow' => 'shadow-blue-500/30'],
                        'purple' => ['gradient' => 'from-purple-500 to-pink-600', 'bg' => 'bg-purple-100/50', 'text' => 'text-purple-600', 'shadow' => 'shadow-purple-500/30'],
                        'emerald' => ['gradient' => 'from-emerald-500 to-teal-600', 'bg' => 'bg-emerald-100/50', 'text' => 'text-emerald-600', 'shadow' => 'shadow-emerald-500/30'],
                        'orange' => ['gradient' => 'from-orange-500 to-amber-600', 'bg' => 'bg-orange-100/50', 'text' => 'text-orange-600', 'shadow' => 'shadow-orange-500/30'],
                        'cyan' => ['gradient' => 'from-cyan-500 to-sky-600', 'bg' => 'bg-cyan-100/50', 'text' => 'text-cyan-600', 'shadow' => 'shadow-cyan-500/30'],
                    ];
                    $colors = $colorConfig[$program->color_theme] ?? $colorConfig['indigo'];
                @endphp
                <a href="{{ route('prokeh.show', $program->slug) }}" class="program-card group">
                    <div class="program-card-inner glow relative bg-white rounded-2xl overflow-hidden shadow-lg border border-gray-100 h-full">
                        ...
                        <div class="pt-8 pb-4 px-6">
                            <div class="program-icon w-20 h-20 mx-auto rounded-2xl bg-gradient-to-br {{ $colors['gradient'] }} flex items-center justify-center shadow-lg {{ $colors['shadow'] }} text-3xl">
                                {{ $program->icon ?? '📚' }}
                            </div>
                        </div>
                        ...
                        <div class="px-6 pb-8 text-center">
                            <!-- Badge -->
                            <div class="inline-block px-3 py-1 {{ $colors['bg'] }} {{ $colors['text'] }} text-xs font-bold rounded-full mb-3">
                                {{ $program->short_name }}
                            </div>
                            
                            <h3 class="text-xl font-bold text-gray-900 mb-3 group-hover:{{ $colors['text'] }} transition-colors">
                                {{ $program->name }}
                            </h3>
                            ...
                            <!-- CTA -->
                            <div class="flex items-center justify-center gap-1 {{ $colors['text'] }} font-semibold text-sm">
                                Lihat Detail
            ...
            <div class="text-center py-16">
                ...
                <p class="text-gray-500 text-lg">Belum ada program keahlian</p>
            </div>
            @endif
        </div>
    </section>
