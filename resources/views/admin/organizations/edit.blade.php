...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-white">Edit Organisasi</h1>
            <p class="text-slate-400 mt-1">{{ $organization->name }}</p>
        </div>
    </div>

    <!-- Form -->
    <form action="{{ route('admin.organizations.update', $organization) }}" method="POST" enctype="multipart/form-data" class="bg-slate-800/50 rounded-xl border border-slate-700 p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="abbreviation" value="{{ old('abbreviation', $organization->abbreviation) }}"
                    class="w-full px-4 py-2.5 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-slate-400 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    placeholder="OSIS">
                @error('abbreviation')
                <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
            ...
            <div>
                ...
                <select name="category" required
                    ...
                    <option value="osis" {{ old('category', $organization->category) === 'osis' ? 'selected' : '' }}>🏛️ OSIS</option>
                    <option value="mpk" {{ old('category', $organization->category) === 'mpk' ? 'selected' : '' }}>⚖️ MPK</option>
                    <option value="pramuka" {{ old('category', $organization->category) === 'pramuka' ? 'selected' : '' }}>⚜️ Pramuka</option>
                    <option value="pmr" {{ old('category', $organization->category) === 'pmr' ? 'selected' : '' }}>🏥 PMR</option>
                    <option value="paskibra" {{ old('category', $organization->category) === 'paskibra' ? 'selected' : '' }}>🎖️ Paskibra</option>
                    <option value="rohis" {{ old('category', $organization->category) === 'rohis' ? 'selected' : '' }}>🕌 Rohis</option>
                    <option value="other" {{ old('category', $organization->category) === 'other' ? 'selected' : '' }}>🎯 Lainnya</option>
                </select>
                @error('category')
                <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="text" name="advisor" value="{{ old('advisor', $organization->advisor) }}"
                    class="w-full px-4 py-2.5 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-slate-400 focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                @error('advisor')
                <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
        ...
        <div>
            ...
            <div x-data="{ logoPreview: '{{ $organization->logo ? asset('storage/' . $organization->logo) : '' }}' }" class="flex items-center gap-4">
        ...
        <div>
            ...
            <div x-data="{ imagePreview: '{{ $organization->image ? asset('storage/' . $organization->image) : '' }}' }" class="border-2 border-dashed border-slate-600 rounded-xl p-6 text-center hover:border-blue-500 transition-colors">
        ...
        <div>
            ...
            <textarea name="description" rows="4"
                class="w-full px-4 py-2.5 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-slate-400 focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none">{{ old('description', $organization->description) }}</textarea>
            @error('description')
            <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="grid md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <textarea name="vision" rows="3"
                    class="w-full px-4 py-2.5 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-slate-400 focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none">{{ old('vision', $organization->vision) }}</textarea>
                @error('vision')
                <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="mission" rows="3"
                    class="w-full px-4 py-2.5 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-slate-400 focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none">{{ old('mission', $organization->mission) }}</textarea>
                @error('mission')
                <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div>
            ...
            <textarea name="achievements" rows="3"
                class="w-full px-4 py-2.5 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-slate-400 focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none">{{ old('achievements', $organization->achievements) }}</textarea>
            @error('achievements')
            <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="grid md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="number" name="order" value="{{ old('order', $organization->order) }}" min="0"
                    class="w-full px-4 py-2.5 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-slate-400 focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                @error('order')
                <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
