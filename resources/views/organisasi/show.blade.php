...
<body class="bg-gray-900 text-gray-100 min-h-screen" x-data>
    ...
    <section class="pt-16 relative">
        <div class="h-[50vh] relative overflow-hidden">
            ...
            <img src="{{ img_url($organization->image, 'organizations', $organization->id, 'image') }}" alt="{{ $organization->name }}"
            ...
            <div class="max-w-5xl mx-auto px-6">
                ...
                <div class="flex items-center gap-6 mb-4">
                    ...
                    <div class="w-20 h-20 bg-white rounded-2xl shadow-2xl p-2 flex items-center justify-center flex-shrink-0">
                        <img src="{{ img_url($organization->logo, 'organizations', $organization->id, 'logo') }}" alt="{{ $organization->abbreviation ?? $organization->name }}" class="w-full h-full object-contain">
                    </div>
                    @else
            ...
            <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <a href="{{ route('organisasi.show', $item->slug) }}"
                    ...
                    <div class="h-40 relative overflow-hidden">
                        @if($item->image)
                        <img src="{{ img_url($item->image, 'organizations', $item->id, 'image') }}" alt="{{ $item->name }}"
                        ...
                        <div class="absolute top-3 left-3 w-10 h-10 bg-white rounded-lg shadow-lg p-1 flex items-center justify-center">
                            <img src="{{ img_url($item->logo, 'organizations', $item->id, 'logo') }}" alt="{{ $item->abbreviation ?? $item->name }}" class="w-full h-full object-contain">
                        </div>
                        @endif
